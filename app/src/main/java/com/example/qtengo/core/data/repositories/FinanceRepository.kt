...
class FinanceRepository {
    ...
    fun getAllFlow(profile: String): Flow<List<FinanceMovement>> = callbackFlow {
        val listener = collection()
            ...
            .addSnapshotListener { snapshot, error ->
                if (error != null) { Log.e("FinanceRepository", "Error: ${error.message}"); trySend(emptyList()); return@addSnapshotListener }
    ...
    suspend fun insert(movement: FinanceMovement) {
        try { collection().add(movement).await() }
        catch (e: Exception) { Log.e("FinanceRepository", "Error insertando: ${e.message}") }
    }

    suspend fun delete(movementId: String) {
        try { collection().document(movementId).delete().await() }
        catch (e: Exception) { Log.e("FinanceRepository", "Error eliminando: ${e.message}") }
    }
}